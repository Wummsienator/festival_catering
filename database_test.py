...
class Database:
    ...
    def getproductsForStand(self, stand):
        products = []
        select = f"SELECT p.* FROM Products AS p INNER JOIN Stand2Product AS s2p ON s2p.ProductID = p.ProductID WHERE s2p.StandID = {stand}"
    ...
    def getOrdersForTicket(self, ticket):
        orders = []
        select = f"SELECT o.*, s.StatusText FROM Orders AS o INNER JOIN Order2Ticket AS o2t ON o2t.OrderID = o.OrderID INNER JOIN Status AS s ON s.StatusID = o.StatusID WHERE o2t.TicketNR = {ticket}"
        for row in self._cursor.execute(select):
